...
import qualified Knit.Effect.AtomicCache as Knit hiding (retrieveOrMake)
...
storeByteStreamDict =
  Knit.SerializeDict
  Store.encode
  (first peekExceptionToSerializationError . Store.decode)
  id
  id
...
listLoader :: (Knit.KnitEffects q, CacheEffects q) => Knit.Sem q [Int]
listLoader = Knit.ignoreCacheTimeM listLoaderWC

listLoaderWC :: (Knit.KnitEffects q, CacheEffects q)
...
listLoader2 ::(Knit.KnitEffects q, CacheEffects q)
              => Knit.Sem q [Int]
