...
addHvega'
  ...
  -> GV.VegaLite
  -> P.Sem effs T.Text
addHvega' idTextM captionTextM svg vl = do
  PE.require PE.VegaSupport
  idText <- maybe (KUI.getNextUnusedId "figure") return idTextM
  addBlaze $ placeVisualization idText captionTextM svg vl
  return idText
...
addHvega
  ...
  -> GV.VegaLite
  -> P.Sem effs T.Text
addHvega idTextM captionTextM vl = addHvega' idTextM captionTextM False vl

-- | Build (Blaze) Html for  hvega visualization with the given id
placeVisualization :: T.Text -> Maybe T.Text -> Bool -> GV.VegaLite -> BH.Html
placeVisualization idText captionTextM svg vl =
